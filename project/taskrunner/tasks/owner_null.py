...
class OwnerNullTask(object):
    ...
    def update_tweets(self, chorus_id):
        UPDATE = """
        UPDATE `vox-data-lake.chorus_db_prod.tweets`
        SET owner_id = NULL WHERE owner_id = {chorus_id};
        """
        ...
        updated_rows = int(query_job.num_dml_affected_rows)
    ...
    def run(self, **kwargs):
        ...
        if chorus_id:
            try:
                (update, cost) = self.update_tweets(chorus_id)
                status = 200
                if update:
                    message = f"Chorus user id nullified in `vox-data-lake.chorus_db_prod.tweets`"
                else:
                    message = 'Chorus id not found in `vox-data-lake.chorus_db_prod.tweets`'
